//Linear search
//traverse through the list and look for a key
// if == key, return data, if not found - return nullptr
...
template<typename T>
Node<T>* SearchKey(Node<T>*& head, int key) {
    Node<T>* temp = head;
    while(temp != nullptr) {
        if (key == temp->data) {
            return (temp);
...
int main() {
    ...
    Node<int>* searchingResult = SearchKey(singlyLL, 20);
